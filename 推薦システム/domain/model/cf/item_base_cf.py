...
class ItemBaseCF(BaseEstimator):

    def fit(self, rating_df: RatingDataFrame):
        self.rating_df = rating_df.copy()
        self.item_similarity_df = self._similarity_of_items(rating_df)
    ...
    def _similarity_of_items(self, rating_df: RatingDataFrame) -> pd.DataFrame:
        """
    ...
    def _similarity_item_ids(self, item_id, user_id) -> pd.Index:
        """
        ユーザ(user_id)によって評価されたアイテム(item_id)と似ているアイテムの集合を取得する.
        """
        is_already_rated_item = self.rating_df[item_id].notnull()
        user_id_already_rated = self.rating_df.loc[is_already_rated_item].index
        similarity = self.item_similarity_df.loc[user_id, user_id_already_rated]
        return similarity.sort_values(ascending=False).index.tolist()

    def predict(self, user_id, item_id) -> float:
        other_item_ids = self._similarity_item_ids(item_id, user_id)
        weight_ser = self.item_similarity_df.loc[user_id, other_user_ids]

        base_rating_to_item = self.mean_rating_to_item_ser.loc[item_id]
        y_lj = self.rating_df.loc[other_user_ids, item_id]
        y_l = self.mean_rating_of_user_ser.loc[other_user_ids]

        return base_rating_to_item + (

...
class PoissonRMLogLikelihood(object):
    """ポアソン回帰モデルの対数尤度関数を定義. ニュートン・ラフソン法でパラメータを推定を行う."""
...
class PoissonRegressionModel(AbstractSAThresholdPredicter):
    ...
    def fit(self, X, y):
        ...
        log_likelihood = PoissonRMLogLikelihood(X, event, time, self.alpha)
